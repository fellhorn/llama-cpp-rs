...
impl LlamaContext<'_> {
    ...
    pub fn save_session_file(
        ...
        if unsafe {
            fellhorn_llama_cpp_sys_2::llama_save_session_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                tokens.as_ptr().cast::<fellhorn_llama_cpp_sys_2::llama_token>(),
                tokens.len(),
            )
        } {
            Ok(())
    ...
    pub fn load_session_file(
        ...
        let tokens_out = tokens.as_mut_ptr().cast::<fellhorn_llama_cpp_sys_2::llama_token>();

        let load_session_success = unsafe {
            fellhorn_llama_cpp_sys_2::llama_load_session_file(
                self.context.as_ptr(),
                cstr.as_ptr(),
                tokens_out,
                max_tokens,
    ...
    pub fn get_state_size(&self) -> usize {
        unsafe { fellhorn_llama_cpp_sys_2::llama_get_state_size(self.context.as_ptr()) }
    }
    ...
    pub unsafe fn copy_state_data(&self, dest: *mut u8) -> usize {
        unsafe { fellhorn_llama_cpp_sys_2::llama_copy_state_data(self.context.as_ptr(), dest) }
    }
    ...
    pub unsafe fn set_state_data(&mut self, src: &[u8]) -> usize {
        unsafe { fellhorn_llama_cpp_sys_2::llama_set_state_data(self.context.as_ptr(), src.as_ptr()) }
    }
}
