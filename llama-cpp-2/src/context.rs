...
pub struct LlamaContext<'a> {
    pub(crate) context: NonNull<fellhorn_llama_cpp_sys_2::llama_context>,
...
impl<'model> LlamaContext<'model> {
    pub(crate) fn new(
        ...
        llama_context: NonNull<fellhorn_llama_cpp_sys_2::llama_context>,
        embeddings_enabled: bool,
    ) -> Self {
        Self {
    ...
    pub fn n_batch(&self) -> u32 {
        unsafe { fellhorn_llama_cpp_sys_2::llama_n_batch(self.context.as_ptr()) }
    }

    /// Gets the max number of physical tokens (hardware level) to decode in batch. Must be less than or equal to [`Self::n_batch`].
    #[must_use]
    pub fn n_ubatch(&self) -> u32 {
        unsafe { fellhorn_llama_cpp_sys_2::llama_n_ubatch(self.context.as_ptr()) }
    }

    /// Gets the size of the context.
    #[must_use]
    pub fn n_ctx(&self) -> u32 {
        unsafe { fellhorn_llama_cpp_sys_2::llama_n_ctx(self.context.as_ptr()) }
    }
    ...
    pub fn decode(&mut self, batch: &mut LlamaBatch) -> Result<(), DecodeError> {
        let result =
            unsafe { fellhorn_llama_cpp_sys_2::llama_decode(self.context.as_ptr(), batch.llama_batch) };
    ...
    pub fn encode(&mut self, batch: &mut LlamaBatch) -> Result<(), EncodeError> {
        let result =
            unsafe { fellhorn_llama_cpp_sys_2::llama_encode(self.context.as_ptr(), batch.llama_batch) };
    ...
    /// - If the current model had a pooling type of [`fellhorn_llama_cpp_sys_2::LLAMA_POOLING_TYPE_NONE`]
    ...
    pub fn embeddings_seq_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = fellhorn_llama_cpp_sys_2::llama_get_embeddings_seq(self.context.as_ptr(), i);
    ...
    pub fn embeddings_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = fellhorn_llama_cpp_sys_2::llama_get_embeddings_ith(self.context.as_ptr(), i);
    ...
    pub fn get_logits(&self) -> &[f32] {
        let data = unsafe { fellhorn_llama_cpp_sys_2::llama_get_logits(self.context.as_ptr()) };
    ...
    pub fn get_logits_ith(&self, i: i32) -> &[f32] {
        ...
        let data = unsafe { fellhorn_llama_cpp_sys_2::llama_get_logits_ith(self.context.as_ptr(), i) };
        let len = usize::try_from(self.model.n_vocab()).expect("n_vocab does not fit into a usize");
    ...
    pub fn reset_timings(&mut self) {
        unsafe { fellhorn_llama_cpp_sys_2::llama_perf_context_reset(self.context.as_ptr()) }
    }

    /// Returns the timings for the context.
    pub fn timings(&mut self) -> LlamaTimings {
        let timings = unsafe { fellhorn_llama_cpp_sys_2::llama_perf_context(self.context.as_ptr()) };
    ...
    pub fn lora_adapter_set(
        ...
        let err_code = unsafe {
            fellhorn_llama_cpp_sys_2::llama_set_adapter_lora(
                self.context.as_ptr(),
    ...
    pub fn lora_adapter_remove(
        ...
        let err_code = unsafe {
            fellhorn_llama_cpp_sys_2::llama_rm_adapter_lora(
                self.context.as_ptr(),
                adapter.lora_adapter.as_ptr(),
            )
        };
...
impl Drop for LlamaContext<'_> {
    fn drop(&mut self) {
        unsafe { fellhorn_llama_cpp_sys_2::llama_free(self.context.as_ptr()) }
    }
}
