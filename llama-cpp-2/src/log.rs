...
fn meta_for_level(
    ...
    match level {
        fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_DEBUG => (&DEBUG_META, &DEBUG_FIELDS),
        fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_INFO => (&INFO_META, &INFO_FIELDS),
        fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_WARN => (&WARN_META, &WARN_FIELDS),
        fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_ERROR => (&ERROR_META, &ERROR_FIELDS),
        _ => {
...
pub(super) struct State {
    ...
    module: Module,
    buffered: std::sync::Mutex<Option<(fellhorn_llama_cpp_sys_2::ggml_log_level, String)>>,
...
impl State {
    ...
    fn generate_log(target: Module, level: fellhorn_llama_cpp_sys_2::ggml_log_level, text: &str) {
    ...
    pub(super) fn cont_buffered_log(&self, text: &str) {
        ...
        if let Some((previous_log_level, mut buffer)) = lock.take() {
            ...
            let level = self
                ...
                as fellhorn_llama_cpp_sys_2::ggml_log_level;
            tracing::warn!(
    ...
    pub(super) fn buffer_non_cont(&self, level: fellhorn_llama_cpp_sys_2::ggml_log_level, text: &str) {
        debug_assert!(!text.ends_with('\n'));
        debug_assert_ne!(level, fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_CONT);
    ...
    pub(super) fn emit_non_cont_line(&self, level: fellhorn_llama_cpp_sys_2::ggml_log_level, text: &str) {
        debug_assert!(text.ends_with('\n'));
        debug_assert_ne!(level, fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_CONT);
        ...
        match level {
            fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_NONE => {
                // TODO: Support logging this to stdout directly via options?
                tracing::info!(no_log_level = true, text);
            }
            fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_DEBUG
            | fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_INFO
            | fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_WARN
            | fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_ERROR => Self::generate_log(self.module, level, text),
            fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_CONT => unreachable!(),
            _ => {
                tracing::warn!(
                    level = level,
                    text = text,
    ...
    pub(super) fn is_enabled_for_level(&self, level: fellhorn_llama_cpp_sys_2::ggml_log_level) -> bool {
        // CONT logs do not need to check if they are enabled.
        let level = if level == fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_CONT {
            self.previous_level
...
mod tests {
    ...
    fn cont_disabled_log() {
        ...
        logs_to_trace(
            fellhorn_llama_cpp_sys_2::GGML_LOG_LEVEL_CONT,
            c"world\n".as_ptr(),
            log_ptr,
        );
